    ...
    store.addCamera(camera, (err, result) => {
      if (err) {
        this.setState({
          message: {
            type: "error",
            msg: _.get(err, "response.data.error.message", "An error"),
          },
        });
      } else {
        this.setState(
          {
            message: {
              type: "success",
              msg: `${camera.name} has been created.`,
            },
          },
          () => {
      ...
      <AddCameraWrapper>
        ...
        <Form onSubmit={this._onSubmit}>
          {msg && messageType === "success" ? (
            <FormSuccessMessage>{msg}</FormSuccessMessage>
          ) : null}
          ...
          <FormCheckboxItem>
            <FormInput
              id={"is-camera-public"}
              value={_.get(camera, "public", false)}
              name={"public"}
              type={"checkbox"}
              onChange={this._onCheckboxFieldChange}
            />
            <FormLabel htmlFor={"is-camera-public"}>
              Is public camera ?
            </FormLabel>
          </FormCheckboxItem>
          <FormAction>
